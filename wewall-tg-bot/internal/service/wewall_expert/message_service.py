...
class WewallExpertMessageService(interface.IWewallExpertMessageService):
    def __init__(
            self,
    ...
    async def handler(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "WewallExpertMessageService.handler",
    ...
    async def __to_manager(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "WewallExpertMessageService.__to_manager",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
    ...
    async def __to_estate_finance_model_expert(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "WewallExpertMessageService.__to_estate_finance_model_expert",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                await self.estate_expert_client.delete_all_message(message.chat.id)
    ...
    async def __to_estate_expert(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "WewallExpertMessageService.__to_estate_expert",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                await self.estate_expert_client.delete_all_message(message.chat.id)
    ...
    async def __to_estate_search_expert(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "WewallExpertMessageService.__to_estate_search_expert",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                await self.estate_expert_client.delete_all_message(message.chat.id)

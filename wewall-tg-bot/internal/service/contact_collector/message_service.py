...
class ContactCollectorMessageService(interface.IContactCollectorMessageService):
    def __init__(
            self,
    ...
    async def handler(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "ContactCollectorMessageService.handler",
            ...
            try:
                ...
                if common.StateSwitchCommand.to_manager in llm_response:
                    ...
                    self.logger.info("Получена команда перехода на эксперт по финансовой модели")
    ...
    async def __to_manager(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "ContactCollectorMessageService.__to_manager",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                ...
                await self.estate_expert_client.delete_all_message(state.tg_chat_id)
    ...
    async def __to_estate_finance_model(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "ContactCollectorMessageService.__to_estate_finance_model",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
    ...
    async def __to_wewall_expert(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "ContactCollectorMessageService.__to_wewall_expert",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                await self.estate_expert_client.delete_all_message(message.chat.id)
    ...
    async def __to_estate_expert(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "ContactCollectorMessageService.__to_estate_expert",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                await self.estate_expert_client.delete_all_message(message.chat.id)
    ...
    async def __to_estate_search_expert(self, message: Message, state: model.State):
        with self.tracer.start_as_current_span(
                "ContactCollectorMessageService.__to_estate_search_expert",
                kind=SpanKind.INTERNAL
        ) as span:
            try:
                await self.estate_expert_client.delete_all_message(message.chat.id)
